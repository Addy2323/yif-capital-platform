...
export function Header() {
  ...
  const showBackButton = user && pathname !== "/"

  return (
    <header className="bg-navy sticky top-0 z-50 transition-all duration-300 border-b border-white/5">
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center gap-4">
            {showBackButton && (
              <Link
                href="/dashboard"
                className="flex items-center justify-center h-8 w-8 rounded-full bg-white/5 border border-white/10 hover:border-gold/30 hover:bg-white/10 transition-all group/back"
                title="Return to Dashboard"
              >
                <ArrowLeft className="h-4 w-4 text-white/70 group-hover/back:text-gold transition-colors" />
              </Link>
            )}
