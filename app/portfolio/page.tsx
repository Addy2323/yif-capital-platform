...
const DSE_STOCKS = [
    ...
    { ticker: "JATU", name: "Jenga Afya Tokomeza Umasi", sector: "Health", price: 0, change: 0.00 },
];
...
export default function PortfolioPage() {
    ...
    const addStock = () => {
        ...
        const cat = DSE_STOCKS.find(s => s.ticker === sTicker.toUpperCase());
        const stock = { id: uid(), ticker: sTicker.toUpperCase(), qty: +sQty, buyPrice: +sBuyPrice, currentPrice: +sCurPrice, dividend: +sDividend || 0, buyDate: sBuyDate, addedAt: Date.now(), name: cat?.name || sTicker, sector: cat?.sector || "Other" };
    ...
    const metrics = (() => {
        ...
        const sv = stocks.reduce((a: number, s: any) => a + s.qty * s.currentPrice, 0);
        const si = stocks.reduce((a: number, s: any) => a + s.qty * s.buyPrice, 0);
        const fv = funds.reduce((a: number, f: any) => a + f.currentValue, 0);
        const fi = funds.reduce((a: number, f: any) => a + f.invested, 0);
        const div = stocks.reduce((a: number, s: any) => a + s.qty * s.dividend, 0);
        const total = sv + fv; const invested = si + fi; const gain = total - invested;
        const gainPct = invested ? ((gain / invested) * 100).toFixed(2) : "0";
        const best = [...stocks].sort((a: any, b: any) => +pctChange(b.currentPrice, b.buyPrice) - +pctChange(a.currentPrice, a.buyPrice))[0];
        return { sv, si, fv, fi, div, total, invested, gain, gainPct, best };
    })();
    ...
    if (authLoading || loading) return (
        <div className="flex min-h-screen flex-col">
            ...
            <main className="flex-1">
                <div style={{ minHeight: "80vh", background: "#0A1F44", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ color: "#D4A017", fontFamily: "'Outfit', sans-serif", fontSize: 18 }}>Loading…</div>
                </div>
            </main>
            <Footer />
        </div>
    ...
    if (!user) return (
        <div className="flex min-h-screen flex-col">
            <Header />
            <main className="flex-1">
                <div style={{ minHeight: "80vh", background: "#0A1F44", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ color: "#D4A017", marginBottom: 20, display: "flex", justifyContent: "center" }}><IconLock /></div>
        ...
        <div className="flex min-h-screen flex-col">
          ...
          `}</style>
                    ...
                    <div className="pf-wrap">
                        {/* ─── TOP NAV ─── */}
                        <div className="flex flex-wrap items-center justify-between gap-y-3 gap-x-2 sticky z-50" style={{ background: "#051430", borderBottom: "1px solid #24427E", padding: "12px 16px", top: 64 }}>
