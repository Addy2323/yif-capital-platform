...
function buildLlmFailureReply(status: number, providerMessage: string): string {
  ...
  const modelHint =
    "Get a key from https://aistudio.google.com/apikey. Remove GEMINI_MODEL from .env to use the default (gemini-3-flash-preview), restart the server, and confirm your key is active."
